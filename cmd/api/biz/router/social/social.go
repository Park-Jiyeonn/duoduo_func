...
import (
	...
	social "simple_tiktok/cmd/api/biz/handler/social"
)
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_message := _douyin.Group("/message", _messageMw()...)
			{
				...
				_action.POST("/", append(_message_ctionMw(), social.MessageAction)...)
			}
			{
				_chat := _message.Group("/chat", _chatMw()...)
				_chat.GET("/", append(_messagechatMw(), social.MessageChat)...)
			}
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action0.POST("/", append(_follow_ctionMw(), social.FollowAction)...)
			}
			{
				_follow := _relation.Group("/follow", _followMw()...)
				{
					_list := _follow.Group("/list", _listMw()...)
					_list.GET("/", append(_getfollowlistMw(), social.GetFollowList)...)
				}
			}
			{
				_follower := _relation.Group("/follower", _followerMw()...)
				{
					...
					_list0.GET("/", append(_getfollowerlistMw(), social.GetFollowerList)...)
				}
			}
			{
				_friend := _relation.Group("/friend", _friendMw()...)
				{
					...
					_list1.GET("/", append(_getfriendlistMw(), social.GetFriendList)...)
				}
			}
		}
	}
